...
async function run() {
  ...
  // --- One sample complaint, so /api/reports and /api/analytics have data ---
  const citizen = await User.findOne({ email: "citizen@demo.com" });
  await Complaint.findOneAndUpdate(
    { reportedBy: citizen._id, channel: "whatsapp", category: "Digital Arrest Scam" },
    {
      ...
      confidence: 91,
      signals: [{ label: "References 'digital arrest'", weight: 34 }],
      recommendedActions: ["Call the National Cyber Crime Helpline 1930."],
      status: "escalated",
      financialLoss: 45000,
      location: { state: "West Bengal", district: "Kolkata", lat: 22.5726, lng: 88.3639 },
    },
    { upsert: true, new: true, setDefaultsOnInsert: true }
  );
