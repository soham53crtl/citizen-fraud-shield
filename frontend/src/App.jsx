...
/* ---------------------------------------------------------------
   RISK ENGINE (heuristic stand-in for the Gemini/Claude classifier
   described in the architecture doc — same input/output contract,
   swap the function body for a real model call server-side)
------------------------------------------------------------------ */
...
function analyzeText(raw) {
  ...
  return { score, hits, category, confidence, band, actions };
}
...
function ForceGraph({ onSelect, selected, highlight }) {
  const [positions, setPositions] = useState(null);
  const dims = { w: 720, h: 420 };

  useEffect(() => {
    const nodes = GRAPH_NODES.map((n) => ({ ...n }));
    const links = GRAPH_LINKS.map(([s, t]) => ({ source: s, target: t }));
    const sim = d3
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 24, alignItems: "center" }}>
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <Pill color={C.sub}><Clock size={12} /> analysed just now</Pill>
          </div>
        </div>
        <button
    ...
    <div>
      <Panel>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1 }} />
          <button
            onClick={() => setResult(analyzeText(text))}
            disabled={!text.trim()}
            style={{
          ...
          >
            <ShieldAlert size={15} style={{ marginRight: 6, verticalAlign: -3 }} /> ANALYZE MESSAGE
          </button>
        </div>
      </Panel>
...
function GraphTab() {
  ...
  const highlight = useMemo(() => {
    ...
    const matchIds = new Set(GRAPH_NODES.filter((n) => n.label.toLowerCase().includes(q)).map((n) => n.id));
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 14 }}>
      <Panel>
        ...
        <ForceGraph onSelect={setSelected} selected={selected} highlight={highlight} />
        ...
        {node && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              {React.createElement(TYPE_META[node.type].icon, { size: 16, color: TYPE_META[node.type].color })}
            ...
            {GRAPH_LINKS.filter(([s, t]) => s === node.id || t === node.id).map(([s, t], i) => {
...
const CHANNELS = [
  ...
  { id: "notice", label: "Notice / PDF", icon: Upload },
];
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      <Panel>
        ...
        <SectionLabel>
          {channel === "url" ? "2. Paste the suspicious URL" : channel === "notice" ? "2. Paste the notice text (or describe the uploaded file)" : "2. Paste the message content"}
        </SectionLabel>
        <textarea
        ...
        >
          <Siren size={15} style={{ marginRight: 6, verticalAlign: -3 }} /> RUN AI FRAUD CHECK
        </button>
      </Panel>
      <Panel>
